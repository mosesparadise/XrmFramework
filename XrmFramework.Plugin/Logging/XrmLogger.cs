...
{
    public class XrmLogger : ILogger
    {
        private readonly IOrganizationService _service;
        private readonly ILoggerContext _context;
        private readonly LogMethod _logMethod;

        public XrmLogger(IOrganizationService service, ILoggerContext context, LogMethod logMethod)
        {
            _service = service;
            _context = context;
        ...
        public void LogWithMethodName(string methodName, string message, params object[] formatArgs)
        {
            LogInternal($"{methodName} : {message}", FormatArgs(formatArgs));
        }
            ...
            if (formatArgs != null)
            {
                ...
                for (var i = 0; i < formatArgs.Length; i++)
                {
                    ...
                    if (formatArgs[i] != null)
                    {
                        ...
                        {
                            formattedArg = $"Count({argType.GetMethod("Count")!.Invoke(formattedArg, new object[] { })})";
                        }
                    }
